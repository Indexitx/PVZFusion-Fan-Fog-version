        ...
        foreach (GameObject gameObject in plantArray)
        {
            ...
            if ((float)component.thePlantColumn == vector.x && (float)component.thePlantRow == vector.y)
            {
                switch (component.thePlantType)
                {
                    case 1093:
                        component.Die();
                        GameAPP.board.GetComponent<CreatePlant>().SetPlant(component.thePlantColumn, component.thePlantRow, 1104);
                        Object.Destroy(base.gameObject);
                        break;
                    case 1098:
                        component.Die();
                        GameAPP.board.GetComponent<CreatePlant>().SetPlant(component.thePlantColumn, component.thePlantRow, 1107);
                        Object.Destroy(base.gameObject);
                        break;
                }
