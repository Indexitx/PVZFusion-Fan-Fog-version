...
public class DriverZombie : Zombie
{
	...
	public override void SetWatered(float time)
	{
		if (timeToWaterApply <= 0)
		{
            if (isDying)
            {
                return;
	...
	protected virtual void DriverPositionUpdate()
	{
		...
		base.transform.Translate((0f - currentSpeed) * Time.deltaTime * freezeSpeed, 0f, 0f);
		...
		if(timeToWaterApply > 0)
		{
            timeToWaterApply -= Time.deltaTime;
		}
	}
	...
	protected override void OnTriggerStay2D(Collider2D collision)
	{
		if (!isMindControlled && collision.TryGetComponent<Plant>(out var component))
		{
			...
			if (component.thePlantRow == theZombieRow)
			{
				...
				component.ZomboniAttacked(this);
			}
		}
