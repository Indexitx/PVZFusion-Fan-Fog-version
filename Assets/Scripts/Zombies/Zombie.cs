...
public class Zombie : MonoBehaviour
{
    ...
    public float theWaterCountDown;
	...
	public bool haveInstrument;

	public GameObject instrumentObject;

    public bool canBeStealed;
	...
	public bool isEatablest = true;

	public int theSecondArmorBroken;

	public int magnetCactusProjMax;

	public float magnetCactusProj;
	...
	public virtual void Steal()
	{
		...
		{
            TakeDamage(0, theFirstArmorHealth);
            TakeDamage(0, theSecondArmorHealth);
        }
    }
	...
	public virtual void CloverAttacked()
	{
		Knockback(0.5f);
	}
	...
	{
        if (GameAPP.difficulty > 4 && !isMindControlled && health > 0)
        {
			health *= 2;
        }
        if (GameAPP.difficulty == 1 && !isMindControlled && health > 0)
        {
            health /= 2;
        }
        flashTime = 0.3f;
		...
		{
			theFirstArmorHealth += health;
		}
		else if(theSecondArmorHealth > 0)
		{
			theSecondArmorHealth += health;
		}
		else
		{
			theHealth += health;
	...
	protected virtual void OnTriggerStay2D(Collider2D collision)
	{
		if (theStatus != 1 && theAttackTarget == null)
		{
			if (!isMindControlled && collision.TryGetComponent<Plant>(out var component))
			{
				...
				{
					if (!TypeMgr.IsCaltrop(component.thePlantType) && !component.isDrone && component.thePlantType != 1105)
					{
	...
	public void SetSplashControl()
	{
		Collider2D[] col = Physics2D.OverlapCircleAll(shadow.transform.position, 1f);
		foreach(Collider2D c in col)
		{
			if(c != null)
			{
				if(c.TryGetComponent<Zombie>(out Zombie zombie))
				{
	...
	public virtual void AttackPlant(Plant plant)
	{
		...
		if (!plant.isAshy)
		{
			plant.TakeDamage(theAttackDamage, this);
	...
	public void Knockback(float l)
	{
		if(transform.position.x + 10 < 18 || l < 0)
		{
            transform.Translate(l, 0f, 0f);
        }
		else
		{
		}
	...
	public virtual void SetWatered(float time)
	{
		if(timeToWaterApply <= 0 && !inWater)
		{
            isJalaed = false;
			isWatered = true;
            theWaterCountDown = time;
            SetColor(base.gameObject, 1);
        }
        
    }

    public virtual void SetAcid(float time)
    {
		if((theFirstArmorHealth > 0) || (theSecondArmorHealth > 0) && !isAcid)
		{
            isJalaed = false;
            if (isWatered)
            {
                isWatered = false;
				TakeDamage(0, theFirstArmorHealth / 5);
            }

            acidTime = time;
            acidCD = 1;
			isAcid = true;
            SetColor(base.gameObject, 5);
        }
    }
