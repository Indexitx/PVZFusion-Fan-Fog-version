...
public class UIMgr : MonoBehaviour
{
	...
	public static void EnterGame(int levelType, int levelNumber, GameObject menu = null)
	{
		...
		GameAPP.board.AddComponent<Board>().SetNumber(levelNumber);
        Time.timeScale = GameAPP.gameSpeed * Board.Instance.theBasicSpeed;
        if (levelType == 3)
		{
			switch (levelNumber)
			{
		...
		gameObject.AddComponent<InitBoard>().SetNumber(levelNumber);
	}
