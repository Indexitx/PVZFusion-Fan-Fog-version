...
public class Board : MonoBehaviour
{
    ...
    public GameObject[] gridPumpkinArray = new GameObject[256];
	...
	public int maxKingCount = 1;

	public int curKingCount = 0;

	public int maxSuperCount = 3;

	public int curSuperCount = 0;

	public Transform METEORStartPos;
	...
	public float NormalMeteorCooldown;

	public float MagnetMeteorCooldown;

	public float UltimateMeteorCooldown;
    ...
    private void Awake()
	{
		...
		theBasicSpeed = 1f;
        NormalMeteorCooldown = -999;
        MagnetMeteorCooldown = -999;
        UltimateMeteorCooldown = -999;
    ...
    private void Start()
	{
		...
		Instantiate(GameAPP.METEORTarget, GameAPP.METEORTarget.transform.position, Quaternion.identity);
        METEORStartPos = Instantiate(GameAPP.METEORStartPos, GameAPP.METEORStartPos.transform.position, Quaternion.identity).transform;
        SetPrePlants();
	}
	...
	{
		if (NormalMeteorCooldown != -999)
		{
			NormalMeteorCooldown -= Time.deltaTime;
			if (NormalMeteorCooldown < 0)
			{
				float i = 0;
				foreach (GameObject g in plantArray)
				{
					if (g == null) continue;
					else
					{
						if (g.TryGetComponent<SuperStarPlant>(out SuperStarPlant j))
						{
							i += j.GetNormalMeteorWeight();
							j.anim.SetTrigger("summon");
                            Debug.Log("SummonMeteor");
                        }
					}
				}
				if (i == 0)
				...
				{
					NormalMeteorCooldown = 60 - i;
					if (NormalMeteorCooldown < 25)
					{
						NormalMeteorCooldown = 25;
					}
					GetComponent<CreateItem>().SetItem(METEORStartPos.position.x, METEORStartPos.position.y, 100);
				}

            }		
		}
        if (MagnetMeteorCooldown != -999)
        {
            MagnetMeteorCooldown -= Time.deltaTime;
            if (MagnetMeteorCooldown < 0)
            {
                float i = 0;
                foreach (GameObject g in plantArray)
                {
                    ...
                    else
                    {
                        if (g.TryGetComponent<SuperStarPlant>(out SuperStarPlant j))
                        {
                            i += j.GetMagnetMeteorWeight();
                            j.anim.SetTrigger("summon");
                            Debug.Log("SummonMagMeteor");
                        }
                    }
                }
                if (i == 0)
                ...
                {
                    MagnetMeteorCooldown = 30;
                    if (MagnetMeteorCooldown < 25)
                    {
                        MagnetMeteorCooldown = 25;
                    }
                    GetComponent<CreateItem>().SetItem(METEORStartPos.position.x, METEORStartPos.position.y, 101);
                }
            }
        }
        if (UltimateMeteorCooldown != -999)
        {
            UltimateMeteorCooldown -= Time.deltaTime;
            if (UltimateMeteorCooldown < 0)
            {
                int i = 0;
                foreach (GameObject g in plantArray)
                {
                    ...
                    {
                        if (g.TryGetComponent<UltimateStarfruit>(out UltimateStarfruit j))
                        {
                            i += j.GetUltimateMeteorWeight();
                            j.anim.SetTrigger("summon");
							Debug.Log("SummonUltMeteor");
                        }
                    }
                }
                if (i == 0)
                ...
                {
					UltimateMeteorCooldown = 20;
                    GetComponent<CreateItem>().SetItem(METEORStartPos.position.x, METEORStartPos.position.y, 102);
                }
            }
        }
    }
	public void SuperStarActivate()
	{
		if(NormalMeteorCooldown == -999)
		{
            NormalMeteorCooldown = 60;
        }
		
	}
    public void SuperMagnetStarActivate()
    {
        if (MagnetMeteorCooldown == -999)
        {
            MagnetMeteorCooldown = 60;
        }
    }
    public void UltimateStarActivate()
    {
        if (UltimateMeteorCooldown == -999)
        {
            UltimateMeteorCooldown = 10;
        }
    }
	...
	public void SetDoom(int theColumn, int theRow, bool setPit, Vector2 position)
	{
		...
		SetCrater(theColumn, theRow);
	}
	public void SetCrater(int theColumn, int theRow)
	{
        GameObject[] array1 = griditemArray;
        foreach (GameObject gameObject in array1)
        {
            if (gameObject != null)
            {
                GridItem component = gameObject.GetComponent<GridItem>();
                if (component.theItemRow == theRow && component.theItemColumn == theColumn)
                {
					return;
                }
            }
        ...
        foreach (GameObject gameObject in array)
        {
            if (gameObject != null)
            {
                ...
                if (component.thePlantRow == theRow && component.thePlantColumn == theColumn && component.thePlantType != 11)
                {
                    component.Die();
                }
            }
    ...
    public void SetWater(int theColumn, int theRow)
    {
		if (boxType[theColumn, theRow] != 1)
		{
            GridItem.CreateGridItem(theColumn, theRow, 3);
        }
		
    }
	public void SetWaterCell(int theColumn, int theRow)
	{
        boxType[theColumn, theRow] = 1;
        GameObject[] array = plantArray;
        foreach (GameObject gameObject in array)
        {
            if (gameObject != null)
            {
                Plant component = gameObject.GetComponent<Plant>();
                if (component.thePlantRow == theRow && component.thePlantColumn == theColumn && !component.isWaterPlant)
                {
                    component.Die();
                }
            }
    ...
    public void ClearWaterCell(int theColumn, int theRow)
    {
        boxType[theColumn, theRow] = 0;
        GameObject[] array = plantArray;
        foreach (GameObject gameObject in array)
        {
            if (gameObject != null)
	...
	public bool YellowFirePea(Bullet bullet, Plant torch, bool fromThreeTorch = false)
	{
		if (bullet.fireLevel < 1)
		{
			...
			{
				bullet.theBulletDamage = bullet.theBulletDamage * 2;
			}
	...
	private void StartNextRound()
	{
		...
		Time.timeScale = theBasicSpeed * GameAPP.gameSpeed;
