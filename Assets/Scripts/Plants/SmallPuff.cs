    ...
    protected override GameObject SearchZombie()
	{
		foreach (GameObject item in board.GetComponent<Board>().zombieArray)
		{
			if (item != null)
			{
				...
				if (!component.isMindControlled && component.theZombieRow == thePlantRow && component.shadow.transform.position.x < 9.2f && component.shadow.transform.position.x > shadow.transform.position.x && component.shadow.transform.position.x < shadow.transform.position.x + 4.5f && SearchUniqueZombie(component) && !component.isFlying)
				{
					return item;
				}
			}
