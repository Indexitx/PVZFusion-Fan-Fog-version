...
public class BloverThornPumpkin : Pumpkin
{
    ...
    public void SummonBlover()
    {
        ...
        gameObject = CreatePlant.Instance.SetPlant(thePlantColumn, thePlantRow, 1085);
        if(gameObject == null)
        {
            column = thePlantColumn + 1;
            if(column <= 9)
            {
                gameObject = CreatePlant.Instance.SetPlant(column, thePlantRow, 1085);
            }
        }
        if (gameObject == null)
        {
        ...
        if (gameObject == null)
        {
            ...
            if (row >= 0)
            {
                gameObject = CreatePlant.Instance.SetPlant(thePlantColumn, row, 1085);
                if (gameObject == null)
                {
                    column = thePlantColumn + 1;
                    if (column <= 9)
                    {
                        gameObject = CreatePlant.Instance.SetPlant(column, row, 1085);
                    }
                }
                if (gameObject == null)
                {
