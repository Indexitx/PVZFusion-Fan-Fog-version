...
public class Mouse : MonoBehaviour
{
	...
	private void LeftClickWithNothing()
	{
		...
		foreach (GameObject item2 in list)
		{
            ...
            if (item2.TryGetComponent<MalletMgr>(out var component7))
            {
                if (!component7.isPickUp && component7.avaliable)
                {
                    component7.PickUp();
                    theItemOnMouse = component7.gameObject;
                    GameAPP.PlaySound(19);
                }
                break;
            }
	...
	private void LeftClickWithSomeThing()
	{
        ...
        else if (theItemOnMouse.name == "Mallet")
        {
            TryToAttack();
        }
    ...
    private void TryToAttack()
    {
        GameObject gameObject = theItemOnMouse;
        ClearItemOnMouse();
		RaycastHit2D raycastHit2D = Physics2D.Raycast(Camera.main.ScreenToWorldPoint(Input.mousePosition), Vector2.zero, LayerMask.GetMask("Zombie"));
        gameObject.GetComponent<MalletMgr>().PutDown();
        if (raycastHit2D.collider != null && raycastHit2D.collider.TryGetComponent<Zombie>(out var component))
		{
			gameObject.GetComponent<MalletMgr>().CD = 0f;
            gameObject.GetComponent<MalletMgr>().Attack(Camera.main.ScreenToWorldPoint((Vector2)Input.mousePosition));
        }
    }
